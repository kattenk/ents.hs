...
startup :: IO SceneState
startup = do
  res <- runScene defaultSceneState $ do
    resource $ Window {
      title = "Flappy",
      res = (144, 256),
      windowSize = ExactWinSize (864, 1536),
      targetFps = 300,
      captureCursor = False,
      exit = False
    }
...
mainLoop :: SceneState -> IO SceneState
mainLoop sceneState = do
  res <- runScene sceneState $ do
    win <- getWindow
    runSystems win
    _ <- userMainLoop
    return ()
...
teardown :: SceneState -> IO ()
teardown _ = putStrLn "tearing down.."
...
shouldClose :: SceneState -> IO Bool
shouldClose _ = return False
